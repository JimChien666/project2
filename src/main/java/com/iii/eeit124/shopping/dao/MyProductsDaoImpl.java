...
//	Integer recordsPerPage = GlobalService.RECORDS_PER_PAGE;
	...
	public List<Products> getPageProducts(Integer pageNo,Integer recordsPerPage,Integer memberId) {
		...
		list = sessionFactory.getCurrentSession().createQuery("from Products where deleted_at=null and status='上架中'  and member_id=?1" + orderByString)
	...
	public Integer getTotalPages(Integer colorId, Integer categoryId, Integer animalTypeId,Integer recordsPerPage,String keywordSearch,Integer memberId) {
		...
		int totalPages = (int) (Math.ceil(getRecordCounts(colorId, categoryId, animalTypeId,keywordSearch,memberId) / (double) recordsPerPage));
		return totalPages;
	}
	...
	public Long getRecordCounts(Integer colorId, Integer categoryId, Integer animalTypeId,String keywordSearch,Integer memberId) {
		...
		//我的商品一定要篩選會員ID
		String condiction = " and member_id = :memberId";
		if (colorId != null) {
...
//			}
		...
		query.setFirstResult(startRecordNo)
			.setMaxResults(recordsPerPage).setParameter("memberId", memberId);
;
...
//  			  .getResultList();
	...
	public Integer getTotalPages(Integer recordsPerPage,Integer memberId) {
		int totalPages = (int) (Math.ceil(getRecordCounts(memberId) / (double) recordsPerPage));
		return totalPages;
	}

	public Long getRecordCounts(Integer memberId) {
		Long count = 0L; // 必須使用 long 型態
		count = (Long) sessionFactory.getCurrentSession().createQuery("SELECT count(*) FROM Products where deleted_at=null and status='上架中' and member_id="+memberId).getSingleResult();
		return count;
	}
...
//		Query<Products> query = sessionFactory.getCurrentSession().createQuery("from Products where deleted_at=null and status='上架中'  and name like ?1 or description like ?2 "+ orderByString, Products.class);
	...
	public List<Products> findMyProducts(Integer memberId,String keywordSearch) {
		TypedQuery<Products> query = sessionFactory.getCurrentSession().createQuery("from Products where member_id=?1 and name like ?2" + orderByString);
		query.setParameter(1, memberId);
		query.setParameter(2, "%"+keywordSearch+"%");
	...
	public List<Products> findMyProducts(Integer memberId) {
		TypedQuery<Products> query = sessionFactory.getCurrentSession().createQuery("from Products where member_id=?1"  + orderByString);
		query.setParameter(1, memberId);
		return query.getResultList();
	}
}
