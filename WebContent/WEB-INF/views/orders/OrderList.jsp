...
<script>
	...
	function showPageOrders(responseData) {
		...
		for (var i = 0; i < orders.length; i++) {
			...
			content += "<tr>" + "<td>" + orders[i].id + "</td>" + "<td>"
					+ orders[i].createdAtString + "</td>" + "<td>"
					+ orders[i].status + "</td>" + "<td>" + orders[i].total
					+ "</td>"
			...
			for (var j = 0; j < orders[i].orderItems.length; j++) {
				...
				content += "<tr>" + "<td>"
						...
						+ "</td>" + "<td>" + orders[i].orderItems[j].status
						+ "</td>" + "</tr>"
	...
	function asynRequest(id) {
		...
		xhr.open("GET", "<c:url value='/order/pagingOrders.json'/>"
				+ queryString, true);
		xhr.send();
		xhr.onreadystatechange = function() {
