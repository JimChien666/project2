...
<script>
	window.onload = function() {
		...
		xhr.open("GET", "<c:url value='/order/pagingSellingOrders.json' />",
				true);
		xhr.send();
	...
	function showPageOrders(responseData) {
		...
		for (var i = 0; i < orders.length; i++) {
			content += "<table>" + "<tr>" + "<th>編號</th>" + "<th>日期</th>"
					+ "<th>狀態</th>" + "<th>總額</th>" + "<th>付款狀態</th>"
					+ "<th>操作</th>" + "</tr>"
			content += "<tr>" + "<td>" + orders[i].id + "</td>" + "<td>"
					+ orders[i].createdAtString + "</td>"
					+ "<td id='order"+orders[i].id+"'>" + orders[i].status
					+ "</td>" + "<td>" + orders[i].total + "</td>"
			...
			content += "<table id='orderInfo"+orders[i].id+"' style='display: none;'>"
					...
					+ "<th>單價</th>"
					+ "<th>數量</th>"
					+ "<th>價格</th>" + "<th>狀態</th>" + "<th>出貨</th>" + "</tr>"
			for (var j = 0; j < orders[i].orderItems.length; j++) {
				console.log(orders[i].orderItems[j]);

				content += "<tr>"
						+ "<td><img width='60' height='80' src='"
						+ imageUrl
						+ "?productId="
						+ orders[i].orderItems[j].productId
						+ "'></td>"
						+ "<td>"
						+ orders[i].orderItems[j].productName
						+ "</td>"
						+ "<td>"
						+ orders[i].orderItems[j].discountPrice
						+ "</td>"
						+ "<td>"
		...
		if (pageNo != 1) {
			...
			navContent += "<li><a id='prev'>" + (parseInt(pageNo) - 1)
					+ "</a></li>";
		} else {
			navContent += "<li>&nbsp;</li>";
			navContent += "<li>&nbsp;</li>";
		}
		navContent += "<li><a class='active' href='#'>" + (parseInt(pageNo))
				+ "</a></li>";
		if (pageNo != totalPage) {
			navContent += "<li><a id='next'>" + (parseInt(pageNo) + 1)
					+ "</a></li>";
	...
	function getOrders() {
		...
		var condiction = "?paidStatus=" + paidStatus + "&orderStatus="
				+ orderStatus;
		var xhr = new XMLHttpRequest();
		xhr.open("GET", "<c:url value='/order/pagingSellingOrders.json' />"
				+ condiction, true);
		...
		if (orderStatus == "") {
			document.getElementById("orderStatus").innerHTML = '<option selected=selected disabled value="">訂單狀態</option><option value="">全部</option><option value="訂單成立">訂單成立</option><option value="已出貨">已出貨</option><option value="取消">取消</option>'
		}
	}
	function shipped(orderItemId) {
		var xhr = new XMLHttpRequest();
		xhr.open("GET", "<c:url value='/order/changeToShipped' />?orderItemId="
				+ orderItemId, true);
		xhr.send();
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4) {
						...
						<div class="shop-topbar-wrapper" style="border: white;">
							<div class="product-sorting-wrapper">
								<div class="product-show shorting-style" id="selectBar"
									...
									</select> <select id="orderStatus" onchange="getOrders()">
